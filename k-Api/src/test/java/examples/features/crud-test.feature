Feature: CRUD Validaciones de API para usuarios
    ...
    Scenario: Create a new user
        ...
        And match response.name == 'Blas Maldonado'
        And match response.status == 'active'

    @UpdateUser
    Scenario: Update an user
        Given path 'users/7517794'
        And request { name: 'Celia Caceres', email: 'pedro.mamania@15ce.com', status: 'active' }
        When method PATCH
        Then status 200
        And match response.name == 'Celia Caceres'
        And match response.status == 'active'

    @DeleteUser
    Scenario: Eliminar un usuario existente
        Given path 'users/7518756'
        When method DELETE
        Then status 204
